    ...
    export default {
      data() {
        return {
          ...
          uploadUrl:'http://8.146.199.110:3000/data/uploadwarmup',
          dialogVisible: false,
          tableData: [],
          currentPage:1,
          pageSize:5,
      ...
      async deleteRow(index, row) {
        ...
        try{
            const res=await this.$axios.post('/data/deldata?type=warmup',data)
                if(res.status==200){
      ...
      async getlesson(){
        try{
            const data= await this.$axios.get('/data/getdata?type=warmup')
              data.data.map(item=>{
      ...
      async handleAddedit(){
        ...
        console.log(data)
        try{
              const res=await this.$axios.post('/data/updatedata?type=warmup',data)
                if(res.status==200){
                  this.$message({ showClose: true,message:res.data.status,type:'success'})
                  this.tableData=[]
                  this.getlesson()
