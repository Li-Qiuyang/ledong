    ...
    export default {
      data() {
        return {
          ...
          uploadUrl:'http://8.146.199.110:3000/data/uploadbodybuildingexercise',
          dialogVisible: false,
          tableData: [],
          currentPage:1,
          pageSize:5,
      ...
      async deleteRow(index, row) {
        ...
        try{
            const res=await this.$axios.post('/data/deldata?type=bodybuildingexercise',data)
                if(res.status==200){
      ...
      async getlesson(){
        try{
            const data= await this.$axios.get('/data/getdata?type=bodybuildingexercise')
              data.data.map(item=>{
      ...
      async handleAddedit(){
        ...
        console.log(data)
        try{
              const res=await this.$axios.post('/data/updatedata?type=bodybuildingexercise',data)
                if(res.status==200){
                  this.$message({ showClose: true,message:res.data.status,type:'success'})
                  this.tableData=[]
                  this.getlesson()
